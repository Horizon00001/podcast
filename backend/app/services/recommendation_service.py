...
POSITIVE_ACTIONS = {"favorite", "like", "play"}
...
class RecommendationService:
    ...
    def get_recommendations(self, user_id: int, limit: int = 20) -> RecommendationResponse:
        ...
        for podcast_id in candidates:
            ...
            reason = self._reason_text(cf, content, hot, fresh)
    ...
    def _reason_text(self, cf: float, content: float, hot: float, fresh: float) -> str:
        reason_signals = {
            "与你历史喜好相似": cf,
            "与你常听内容主题一致": content,
            "近期全站热度较高": hot,
