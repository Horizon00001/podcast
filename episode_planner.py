...
def build_podcast_plan(category: str, items: List[dict]) -> EpisodePlan:
    ...
    selected_items = [
        PlannedNewsItem(
            item_id=item.get("item_id") or _similarity_key(item),
            feed_id=item.get("feed_id") or item.get("feed_name") or "unknown",
            feed_name=item.get("feed_name") or item.get("feed_id") or "Unknown Feed",
            category=category,
            ...
            link=item.get("link", ""),
            score=1.0,
            selection_reason="已通过相似度聚类归入本期播客素材",
        )
        for item in items
    ]
    ...
    if selected_items:
        segments.append(
            EpisodeSegment(
                ...
                purpose="用本组主题建立本期主线和听众期待。",
                item_refs=[selected_items[0].item_id],
                segment_thesis=f"先用最具代表性的新闻引出 {top_story_title} 的节目主线。",
            )
        )
        ...
        for index, item in enumerate(selected_items):
            segments.append(
                EpisodeSegment(
                    ...
                    purpose="展开本组内的一条核心新闻。",
                    item_refs=[item.item_id],
                    segment_thesis=f"把 {item.title} 讲透，作为第 {index + 1} 条核心素材。",
                )
            )
        ...
        segments.append(
            EpisodeSegment(
                ...
                purpose="回收主线，总结本期节目真正想表达的判断。",
                item_refs=[],
                segment_thesis=closing_takeaway,
            )
        )

    title_hint = f"{profile_name} | {top_story_title}"
    return EpisodePlan(
        topic_id=category,
        ...
        audience="泛科技与新闻播客听众",
        editorial_angle="围绕相似新闻聚类出的共同主线展开。",
...
def build_group_plan(category: str, items: List[dict], topic_name: str) -> EpisodePlan:
    ...
    return EpisodePlan(
        ...
        title_hint=f"{topic_name} | {top_story_title}",
        theme_statement=theme_statement,
        audience="泛科技与新闻播客听众",
        editorial_angle="围绕相似新闻聚类出的共同主线展开。",
        selected_items=selected_items,
...
def build_group_name(items: List[dict], fallback: str) -> str:
    title = _group_title(items, fallback)
...
def save_pending_groups(pending_groups: list[dict], used_item_links: list[str], output_path) -> Path:
    ...
    payload = {
        ...
        "used_item_links": used_item_links,
    }
...
def merge_pending_groups(
    ...
    for pending_group in pending_groups:
        ...
        for item in candidates:
            item_key = _similarity_key(item)
            if item.get("link") in {existing.get("link") for existing in pending_items}:
                continue
            item_vec = _tfidf_vectors([item]).get(item_key, {})
...
def load_topic_profiles(config_path) -> Dict[str, TopicProfile]:
    with open(config_path, "r", encoding="utf-8") as f:
        raw_profiles = json.load(f).get("topics", [])
    ...
    for raw in raw_profiles:
        ...
        profile = TopicProfile(
            ...
            audience=raw.get("audience", "泛播客听众"),
            editorial_angle=raw.get("editorial_angle", "围绕主题组织一集节目"),
            ...
            structure_template=raw.get("structure_template", ["opening", "top_story", "closing"]),
            max_items=max(int(selection.get("max_items", 4)), 1),
        )
...
def resolve_topic_profile(topic: str, config_path) -> TopicProfile:
    ...
    return TopicProfile(
        id=normalized,
        name=normalized,
        description=f"围绕 {normalized} 组织节目。",
        audience="泛播客听众",
        editorial_angle=f"所有入选内容都应服务于 {normalized} 这个主题主线。",
        allowed_categories=[],
        preferred_keywords=[normalized],
        excluded_keywords=[],
        structure_template=["opening", "top_story", "related_signals", "closing"],
        max_items=4,
    )
...
def load_rss_items(rss_data_path) -> List[dict]:
    ...
    for feed in feeds:
        for index, entry in enumerate(feed.get("entries", []), start=1):
            items.append(
                {
                    "item_id": f"{feed.get('id', 'feed')}-{index}",
...
def _segment_purpose(segment_type: str, profile: TopicProfile) -> str:
    mapping = {
        "opening": f"用节目主题 {profile.name} 建立本期主线和听众期待。",
        "top_story": "展开最能承载本期主线的核心新闻。",
        "related_signals": "用其他新闻证明这不是孤立事件，而是更大趋势的一部分。",
        "impact": "解释这些变化对听众和行业意味着什么。",
        "developer_impact": "把行业变化翻译成对程序员工具、协作和职业判断的具体影响。",
        "closing": "回收主线，总结本期节目真正想表达的判断。",
    }
...
def build_episode_plan(topic: str, rss_data_path, topics_config_path) -> EpisodePlan:
    ...
    for segment_type in profile.structure_template:
        ...
        segments.append(
            EpisodeSegment(
                ...
                purpose=_segment_purpose(segment_type, profile),
                item_refs=item_refs,
                segment_thesis=thesis,
            )
        )
    ...
    return EpisodePlan(
        ...
        title_hint=f"{profile.name} | {top_story_title}",
        theme_statement=theme_statement,
        audience=profile.audience,
        editorial_angle=profile.editorial_angle,
        selected_items=selected_items,
        segments=segments,
        closing_takeaway=closing_takeaway,
    )
...
def format_plan_for_prompt(plan: EpisodePlan) -> str:
    lines = [
        "以下是已经完成选材和编排的播客节目计划，请严格围绕这个计划写成一集节目，而不是把新闻简单堆砌。",
        f"节目主题: {plan.topic_name} ({plan.topic_id})",
        f"目标听众: {plan.audience}",
        f"节目角度: {plan.editorial_angle}",
        f"标题建议: {plan.title_hint}",
        f"本期主线: {plan.theme_statement}",
        "",
        "已选素材:",
    ]
    for item in plan.selected_items:
        lines.extend(
            [
                ...
                f"  来源: {item.feed_name} / {item.category}",
    ...
    for segment in plan.segments:
        ...
        lines.extend(
            [
                f"- 段落类型: {segment.segment_type}",
                f"  目的: {segment.purpose}",
                f"  使用素材: {refs}",
                f"  这一段要表达的判断: {segment.segment_thesis}",
            ]
        )
